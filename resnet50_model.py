...
def train(model, device, train_loader, valid_loader, optimizer, epoch):
    ...
    for i in range(epoch):
        ...
        if (train_correct + valid_correct) > total_correct:
            total_correct = train_correct + valid_correct
            torch.save(model.state_dict(), 'model_weights.pth')
    fig1 = plt.figure()
    plt.title("loss")
    #plt.plot(epoch_time, loss_t_list, epoch_time, loss_v_list, label='train', label='valid')
    plt.plot(epoch_time, loss_t_list, label='train')
    plt.plot(epoch_time, loss_v_list, label='valid')
    fig1.savefig('loss.png')
    fig2 = plt.figure()
    plt.title("accurancy")
    #plt.plot(epoch_time, acc_t_list, epoch_time, acc_v_list, label='train', label='valid')
...
def module():
    ...
    model = torchvision.models.resnet50(weights=models.ResNet50_Weights.IMAGENET1K_V1, progress=True)
